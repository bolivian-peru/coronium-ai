...
export class Coronium {
  ...
  readonly balance = {
    get: (): Promise<Balance> => this.request<Balance>({ method: "GET", path: "/balance" }),
  };

  readonly deposit = {
    address: (input?: { chain?: Chain; amount_usd?: number }): Promise<DepositAddress> =>
      this.request<DepositAddress>({ method: "POST", path: "/deposit/address", body: input ?? {} }),
  };

  readonly tariffs = {
    list: (input?: { country?: string; carrier?: string; type?: "4g" | "5g" }): Promise<Tariff[]> =>
      this.request<Tariff[]>({ method: "GET", path: "/tariffs", query: input }),
  };

  readonly proxies = {
    list: (): Promise<Proxy[]> => this.request<Proxy[]>({ method: "GET", path: "/proxies" }),
    ...
    release: (id: string): Promise<void> =>
      this.request<void>({ method: "DELETE", path: `/proxies/${encodeURIComponent(id)}` }),
